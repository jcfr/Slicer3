...
template<class DATA, int DIMENSIONALITY, class PRECISION, class SOURCE1,
    class SOURCE2>
class SourceMixedOf: public SourceOf<DATA, DIMENSIONALITY, PRECISION>
{
protected:
  SOURCE1* m_psource1;
  SOURCE2* m_psource2;
...
protected:
  SourceMixedOf() :
    SourceOf<DATA, DIMENSIONALITY, PRECISION> (), m_psource1(NULL), m_psource2(
        NULL)
...
public:
  ...
  {
    return (_LINE + "SourceMixedOf(" + DIMENSIONALITY + "D)" + LINE_
        + this->DescribeCommon());
  }
  ...
  virtual String DescribeCommon() const
  {
    return _INDENT + (_LINE + "size: " + this->Size().Describe() + LINE_
        + (_INDENT + "source 1:" + LINE_ + this->m_psource1->Describe()
...
inline void Get(const SourceMixedOf<DATA, 3, PRECISION, SOURCE1, SOURCE2>& src, DATA& dataOut, const PRECISION& nX, const PRECISION& nY)
{
  src.VirtualGet(dataOut, nX, nY);
}

template<class DATA, class PRECISION, class SOURCE1, class SOURCE2>
inline void Set(SourceMixedOf<DATA, 3, PRECISION, SOURCE1, SOURCE2>& src, const PRECISION& nX, const PRECISION& nY, const DATA& data)
{
  src.VirtualSet(nX, nY, data);
}

template<class DATA, class PRECISION, class SOURCE1, class SOURCE2>
inline void Get(const SourceMixedOf<DATA, 3, PRECISION, SOURCE1, SOURCE2>& src, DATA& dataOut, const PRECISION& nX, const PRECISION& nY, const PRECISION& nZ)
{
  src.VirtualGet(dataOut, nX, nY, nZ);
}

template<class DATA, class PRECISION, class SOURCE1, class SOURCE2>
inline void Set(SourceMixedOf<DATA, 3, PRECISION, SOURCE1, SOURCE2>& src, const PRECISION& nX, const PRECISION& nY, const PRECISION& nZ, const DATA& data)
{
