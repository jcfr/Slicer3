...
public:
  ...
  template<int DIMENSIONALITY2, class PRECISION2>
  PointOf<2, PRECISION>& operator=(const PointOf<DIMENSIONALITY2, PRECISION2> &pt)
  {
    if (DIMENSIONALITY2 >= 1)
      {
      m_xX = pt.Dim(0);
  ...
  PointOf<2, PRECISION>& AdvanceLoop(const PointOf<2, PRECISION>& ptBy = 1)
  {
    m_xX += ptBy.m_xX;
    return *this;
  }

  bool StillLooping(const PointOf<2, PRECISION>& ptTo = 0, const PointOf<2,
      PRECISION>& ptFrom = 0, const PointOf<2, PRECISION>& ptBy = 1)
  {
    if (m_xX < ptTo.X())
      {
      return true;
  ...
  PointOf<2, PRECISION> Bound(const PointOf<2, PRECISION> &ptMin, const PointOf<
      2, PRECISION> &ptMax) const
  {
    return PointOf<2, PRECISION> (::Bound(X(), ptMin.X(), ptMax.X()), ::Bound(
        Y(), ptMin.Y(), ptMax.Y()));
  }
...
//============================================================================
template<class PRECISION>
inline PointOf<2, PRECISION> NewRandomPoint(const PointOf<2, PRECISION> &pt)
{
  return PointOf<2, PRECISION> (Random(PRECISION(0), pt.X()), Random(PRECISION(
      0), pt.Y()));
}
