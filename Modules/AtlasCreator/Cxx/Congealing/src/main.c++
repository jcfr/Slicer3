...
{
  DimensionType dtSlice = DimensionType(ConfigValue("congeal.output.dimension",
      int(DimensionY)));
  int nSlice = ConfigValue("congeal.output.slice", .5)
  ...
  UD1("writing out overview congealed volumes");
    ...
    for (int dtSlice = 0; dtSlice < 3; dtSlice++)
      ...
      int cLayout = min(recipie.CSources(), ConfigValue(
          "congeal.output.sourcegrid", 1024));
      ...
      int cR = floor(sqrt(cLayout));
        ...
        for (int n = 0; n < cLayout; n++)
          ...
          layoutInputs.AddSource(ScaleTo(Point2D(1024. / cC, 1024. / cR),
              Aperature(Point2D(-50., -50.), Point2D(350., 350.), ScaleTo(
                  Point2D(300., 300.), Slice(DimensionType(dtSlice), nSlice, (//										CubicInterpolation(
                      recipie.PSource(n)))))));
          }
        ...
        {
        TICK( dtmWriteVolume);
        WritePGM(StringF(sflPrefix + "out%03d.%03d-inputs%d-%03d.pgm",
            nSchedule, nProgress, dtSlice, nSlice), Rasterize(
            GreyPixelRangeScale(dataMean, dataRange, ScaleTo(Point2D(512, 512),
                &layoutInputs))));
        ...
        WritePGM(StringF(sflPrefix + "out%03d.%03d-average%d-%03d.pgm",
            nSchedule, nProgress, dtSlice, nSlice), Rasterize(ScaleUpto(
            Point2D(ConfigValue("congeal.output.average.width", 512),
                ConfigValue("congeal.output.average.height", 512)),
            GreyPixelRangeScale(dataMean, dataRange, Slice(DimensionType(
                dtSlice), nSlice, (//									CubicInterpolation(
                Average(recipie.VSources(), recipie.CSources())))))));
...
void LoadAndScaleSources(int c, ScanVolume* vscan, const String& sConfig)
{
  ...
  for (int n = 0; n < c; n++)
    ...
    String sInputFormat = ConfigValue(
        StringF("congeal.inputfile[%d].format", n), ConfigValue(
            "congeal.inputfile.format", "brainweb"));
    ...
    String sfl = ConfigValue(StringF("congeal.inputfile[%d]", n), sflDefault);
    ...
    if (sInputFormat == "brainweb")
      ...
      {
      vscan[n] = DereferencePointer(RasterizeDownsample(ConfigValue(sConfig
          + ".downsample", 0), ReadNifti(sfl)));
...
//============================================================================
//============================================================================
void IncreaseWarpfieldControlPointResolution(int c, Recipie_G3R_All_G3I& recipie, const String& sConfig)
{
  ...
  for (int n = 0; n < c; n++)
    {
    IncreaseWarpfieldControlPointResolution(recipie.LayerWarp0(n), PointOf<3,
        Real> (ConfigValue(sConfig + ".warpfield[0].size", 1)),
        InterpolationType(ConfigValue(sConfig + ".warpfield[0].interpolation",
            3)));

    IncreaseWarpfieldControlPointResolution(recipie.LayerWarp1(n), PointOf<3,
        Real> (ConfigValue(sConfig + ".warpfield[1].size", 1)),
        InterpolationType(ConfigValue(sConfig + ".warpfield[1].interpolation",
            3)));

    IncreaseWarpfieldControlPointResolution(recipie.LayerWarp2(n), PointOf<3,
        Real> (ConfigValue(sConfig + ".warpfield[2].size", 1)),
        InterpolationType(ConfigValue(sConfig + ".warpfield[2].interpolation",
            3)));

    IncreaseWarpfieldControlPointResolution(recipie.LayerWarp3(n), PointOf<3,
        Real> (ConfigValue(sConfig + ".warpfield[3].size", 1)),
        InterpolationType(ConfigValue(sConfig + ".warpfield[3].interpolation",
            3)));
    }
}
...
void XCopyInto(SOURCE_OUT *psrcOut, SOURCE_IN *psrcIn, bool bPrepared = false)
{
  ClaimPointer(psrcIn);
...
//============================================================================
//============================================================================
void UpdateScheduleRegistrationParameters(Recipie_G3R_All_G3I& recipie, const String& sConfig)
{

  D("set up schedule registration parameters");
  bool b = ConfigValue(sConfig + ".optimize.affine", true);
  recipie.OptimizeLayerTranslate0(ConfigValue(sConfig
      + ".optimize.translate[0]", b));
  recipie.OptimizeLayerRotate(ConfigValue(sConfig + ".optimize.rotate", b));
  recipie.OptimizeLayerTranslate1(ConfigValue(sConfig
      + ".optimize.translate[1]", b));
...
//============================================================================
//============================================================================
void Congeal(int nSchedule, Recipie_G3R_All_G3I& recipie, const String& sConfig)
{
  ...
  int cSamples = ConfigValue(sConfig + ".optimize.samples", ConfigValue(
      "congeal.optimize.samples", 500));

  int cIterations = ConfigValue(sConfig + ".optimize.iterations", ConfigValue(
      "congeal.optimize.iterations", 1000));

  int cProgress = ConfigValue(sConfig + ".optimize.progresspoints",
  ...
  for (int nProgress = 0; nProgress < cProgress; nProgress++)
    ...
    if (nProgress > 0)
      {
      cg.StatisticalPhasedCongeal(cSamples, .0001, cIterations, (nProgress - 1)
          * cIterationsPerProgress, (nProgress) * cIterationsPerProgress - 1);
      }
    ...
    if (fork() == 0)
      ...
      while (wait3(&zReturn, WNOHANG, 0) != 0);
      }
#endif
    }
}
...
void CongealSchedules()
{
  ...
  for (int n = 0; n < c; n++)
    ...
    Set(*recipie.LayerWarp0(n).GetPWarpControlPoints(), 0, 0, 0,
        Point3DReal(0.));
    Set(*recipie.LayerWarp1(n).GetPWarpControlPoints(), 0, 0, 0,
        Point3DReal(0.));
    Set(*recipie.LayerWarp2(n).GetPWarpControlPoints(), 0, 0, 0,
        Point3DReal(0.));
    Set(*recipie.LayerWarp3(n).GetPWarpControlPoints(), 0, 0, 0,
        Point3DReal(0.));
    }
  ...
  for (int nSchedule = 0; nSchedule < cSchedules; nSchedule++)
    ...
    bool bCache = ConfigValue(sConfig + ".cache", ConfigValue("congeal.cache",
        true));
    ...
    if (bCache && LoadCachedRecipie(sflCache, recipie, vscan))
      {
      UD("Using cached congealing results for recipie for schedule %d/%d",
          nSchedule, cSchedules);
      }
    else
      {
      // do congealing steps
  ...
  while (wait(&zReturn) != -1 && errno != ECHILD);
}
...
void TestSpeed()
{
  ...
  UD("SPEED TEST = %g (%g)", TOCK(dtmOptimize), Real(cSamples) / TOCK(
      dtmOptimize));
  D("%d", z);
}
...
{
  WritePGM("out.pgm", Rasterize(GreyPixelRangeScale(1024, 2048, Slice(
      DimensionZ, 120, ScaleBy(Point3DReal(2.7), Rotate(28, 37, 53, ReadNifti(
          "../input/NIH/uncompressed/deface_1453_V4_t1w.nii")))))));
}
...
int main(int cCLA, char *vsCLA[])
{
  ...
  ASSERTf(ObjectsOutstanding() == cStaticObjects, "Objects leaked %d",
