...
void NormalizeGroup(int csource, SourceTransformRotateOf<DATA, 2, PRECISION,
    SUBSOURCE>* vsrc)
{
  Real rTAngle = 0;
  ...
  for (int n = 0; n < csource; n++)
    {
    vsrc[n].SetAngle(vsrc[n].GetAngle() - rTAngle);
    }
}

template<class DATA, class PRECISION, class SUBSOURCE>
void RegisterInitialSteps(const SourceTransformRotateOf<DATA, 2, PRECISION,
    SUBSOURCE> &src, RegistryOfInitialSteps &regSteps)
{
...
template<class DATA, class PRECISION, class SUBSOURCE>
void NormalizeGroup(int csource, SourceTransformRotateOf<DATA, 3, PRECISION,
    SUBSOURCE>* vsrc)
{
  Real rTAngleX = 0;
  Real rTAngleY = 0;
  ...
  for (int n = 0; n < csource; n++)
    {
    vsrc[n].SetAngle(::Bound(vsrc[n].GetAngleX(), -30., 30.), ::Bound(
        vsrc[n].GetAngleY(), -30., 30.),
  ...
  for (int n = 0; n < csource; n++)
    {
    vsrc[n].SetAngle(vsrc[n].GetAngleX() - rTAngleX, vsrc[n].GetAngleY()
        - rTAngleY, vsrc[n].GetAngleZ() - rTAngleZ);
    }
}

template<class DATA, class PRECISION, class SUBSOURCE>
void RegisterInitialSteps(const SourceTransformRotateOf<DATA, 3, PRECISION,
    SUBSOURCE> &src, RegistryOfInitialSteps &regSteps)
{
  static Real rStepDefault = ConfigValue("congeal.initialsteps.rotate", 1.);
  regSteps.Register(rStepDefault);
...
//============================================================================
template<class DATA, int DIMENSIONALITY, class PRECISION, class SUBSOURCE>
void NormalizeGroup(int csource, SourceTransformScaleOf<DATA, DIMENSIONALITY,
    PRECISION, SUBSOURCE>* vsrc)
{
  ...
  for (int n = 0; n < csource; n++)
    {
    vsrc[n].SetScale(vsrc[n].Scale() / ptScale);
    }
}

template<class DATA, int DIMENSIONALITY, class PRECISION, class SUBSOURCE>
void RegisterInitialSteps(const SourceTransformScaleOf<DATA, DIMENSIONALITY,
    PRECISION, SUBSOURCE> &src, RegistryOfInitialSteps &regSteps)
{
...
//============================================================================
template<class DATA, int DIMENSIONALITY, class PRECISION, class SUBSOURCE>
void NormalizeGroup(int csource, SourceTransformTranslateOf<DATA,
    DIMENSIONALITY, PRECISION, SUBSOURCE>* vsrc)
{
  typedef PointOf<DIMENSIONALITY, Real> POINT;
...
template<class DATA, int DIMENSIONALITY, class PRECISION, class SUBSOURCE>
void RegisterInitialSteps(const SourceTransformTranslateOf<DATA,
    DIMENSIONALITY, PRECISION, SUBSOURCE> &src, RegistryOfInitialSteps &regSteps)
{
...
//============================================================================
template<class DATA, int DIMENSIONALITY, class PRECISION, class SUBSOURCE>
void NormalizeGroup(int csource, SourceTransformWarpOf<DATA, DIMENSIONALITY,
    PRECISION, SUBSOURCE>* vsrc)
{
...
template<class DATA, int DIMENSIONALITY, class PRECISION, class SUBSOURCE>
void RegisterInitialSteps(const SourceTransformWarpOf<DATA, DIMENSIONALITY,
    PRECISION, SUBSOURCE> &src, RegistryOfInitialSteps &regSteps)
{
  typedef PointOf<DIMENSIONALITY, Real> WARPCP;
  typedef PointOf<DIMENSIONALITY, int> POINT;
...
//============================================================================
template<class DATA, int DIMENSIONALITY, class PRECISION, class SUBSOURCE>
void NormalizeGroup(int csource, SourceTransformDataScaleOf<DATA,
    DIMENSIONALITY, PRECISION, SUBSOURCE>* vsrc)
{
  typedef PointOf<DIMENSIONALITY, Real> POINT;
  ...
  for (int n = 0; n < csource; n++)
    {
    vsrc[n].SetSlopeIntercept(::Bound(vsrc[n].Slope(), .2, 5.), ::Bound(
        vsrc[n].Intercept(), -.5, +.5));
    }
  ...
  for (int n = 0; n < csource; n++)
    {
    vsrc[n].SetSlopeIntercept(vsrc[n].Slope() / rSlopeT, vsrc[n].Intercept()
        - rInterceptT);
    }
}

template<class DATA, int DIMENSIONALITY, class PRECISION, class SUBSOURCE>
void RegisterInitialSteps(const SourceTransformDataScaleOf<DATA,
    DIMENSIONALITY, PRECISION, SUBSOURCE> &src, RegistryOfInitialSteps &regSteps)
{
  static Real rStepDefaultSlope = ConfigValue(
      "congeal.initialsteps.datascale.slope", .01);
  static Real rStepDefaultIntercept = ConfigValue(
      "congeal.initialsteps.datascale.intercept", .01);
...
//============================================================================
template<class DATA, int DIMENSIONALITY, class PRECISION, class SUBSOURCE>
void NormalizeGroup(int csource, SourceTransformSpatialScaleOf<DATA,
    DIMENSIONALITY, PRECISION, SUBSOURCE>* vsrc)
{
  typedef PointOf<DIMENSIONALITY, int> POINT;
...
template<class DATA, int DIMENSIONALITY, class PRECISION, class SUBSOURCE>
void RegisterInitialSteps(const SourceTransformSpatialScaleOf<DATA,
    DIMENSIONALITY, PRECISION, SUBSOURCE> &src, RegistryOfInitialSteps &regSteps)
{
  int c = src.GetPCoefficients()->Size().CVolume();
  static Parameter paramStepDefault = ConfigValue(
      "congeal.initialsteps.spatialscale", .01);
  for (int n = 0; n < c; n++)
    {
