...
class RecipieOf: public Counted
{
...
protected:
  // must call
  RecipieOf() :
    Counted(), m_csrc(0), m_vsrcFinal(NULL)
  {
  }
...
public:
  ...
  virtual void RegisterParameters(RegistryOfParameters& regParam) =0;

  virtual void RegisterInitialSteps(RegistryOfInitialSteps& regSteps) =0;
...
public:
  ...
  virtual String Describe() const
  {
    ...
    for (int n = 0; n < m_csrc; n++)
      {
      s = s + (_LINE + "Source " + n + LINE_ + (_INDENT
          + m_vsrcFinal[n].Describe()));
      }

    return (_LINE + "Recipie with " + m_csrc + LINE_ + s);
  }
