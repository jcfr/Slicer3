...
//============================================================================
Parameter NelderMeadOptimize(int cDimensions, Parameter* vrParamMin, Parameter* vrInitialScale, Parameter fxnEval(const Parameter*, const Parameter& rErrMin, const void*), const void* pData, Real rErrThreshold, int cIterations)
{
  ...
  for (;; nIteration++)
    ...
    D1("COMPARING (%g,%g) --> %g and %g", rErrMax, rErrMin, rErrFraction,
        rErrThreshold);
    ...
    for (int nDimension = 0; nDimension < cDimensions; nDimension++)
      {
      vrParamReflection[nDimension] = (vrParamCentroid[nDimension]
          + (vrParamCentroid[nDimension] - vvrParam[nPointMax][nDimension])
              * rFactorReflection);
      }
    ...
    if (rErrReflection <= rErrMin)
      ...
      for (int nDimension = 0; nDimension < cDimensions; nDimension++)
        {
        vrParamExpansion[nDimension] = (vrParamCentroid[nDimension]
            + (vrParamCentroid[nDimension] - vvrParam[nPointMax][nDimension])
                * rFactorExpansion);
        }
    ...
    for (int nDimension = 0; nDimension < cDimensions; nDimension++)
      {
      vrParamContraction[nDimension] = (vvrParam[nPointMax][nDimension]
          + (vrParamCentroid[nDimension] - vvrParam[nPointMax][nDimension])
              * rFactorContraction);
      }
    ...
    for (int nPoint = 0; nPoint < cPoints; nPoint++)
      ...
      for (int nDimension = 0; nDimension < cDimensions; nDimension++)
        {
        vvrParam[nPoint][nDimension] = (vvrParam[nPointMin][nDimension]
            + (vvrParam[nPoint][nDimension] - vvrParam[nPointMin][nDimension])
                * rFactorReduction);
        }
      vrErr[nPoint] = fxnEval(vvrParam[nPoint], DBL_MAX, pData);
