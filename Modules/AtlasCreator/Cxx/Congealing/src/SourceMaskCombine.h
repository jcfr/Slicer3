...
template<class DATA, int DIMENSIONALITY, class PRECISION, class SOURCEMASK,
    class SOURCECOMBINE>
class SourceMaskCombineOf: public SourceOf<DATA, DIMENSIONALITY, PRECISION>
{
...
protected:
  SourceMaskCombineOf() :
    SourceOf<DATA, DIMENSIONALITY, PRECISION> (), m_psourceMask(NULL),
        m_vecpsrc()

  {
  }

  SourceMaskCombineOf(SourceMaskCombineOf<DATA, DIMENSIONALITY, PRECISION,
      SOURCEMASK, SOURCECOMBINE>&src) :
    SourceOf<DATA, DIMENSIONALITY, PRECISION> (src), m_vecpsrc(src.m_vecpsrc)
  {
...
public:
  ...
  {
    return (_LINE + "SourceMaskCombineOf(" + DIMENSIONALITY + "D)" + LINE_
        + this->DescribeCommon());
  }
  ...
  virtual String DescribeCommon() const
  {
    ...
    for (int n = 0; n < m_vecpsrc.C(); n++)
      {
      s = s + (_INDENT + (_LINE + "sourcecombine: " + n + LINE_ + (_INDENT
          + this->m_vecpsrc[n]->Describe() + LINE_)));
      }

    return _INDENT + (_LINE + "size: " + this->Size().Describe() + LINE_
        + (_INDENT + "source mask:" + LINE_ + this->m_psourceMask->Describe())
        + s);
  }
  ...
  virtual PointOf<DIMENSIONALITY, PRECISION> Bound(const PointOf<
      DIMENSIONALITY, PRECISION>& pt) const
  {
    return pt.Bound(PointOf<DIMENSIONALITY, PRECISION> (PRECISION(0)),
        this->Size());
  }
...
inline void Get(const SourceMaskCombineOf<DATA, 3, PRECISION, SOURCEMASK,
    SOURCECOMBINE>& src, DATA& dataOut, const PRECISION& nX, const PRECISION& nY)
{
  src.VirtualGet(dataOut, nX, nY);
}

template<class DATA, class PRECISION, class SOURCEMASK, class SOURCECOMBINE>
inline void Set(SourceMaskCombineOf<DATA, 3, PRECISION, SOURCEMASK,
    SOURCECOMBINE>& src, const PRECISION& nX, const PRECISION& nY, const DATA& data)
{
...
template<class DATA, class PRECISION, class SOURCEMASK, class SOURCECOMBINE>
inline void Get(const SourceMaskCombineOf<DATA, 3, PRECISION, SOURCEMASK,
    SOURCECOMBINE>& src, DATA& dataOut, const PRECISION& nX, const PRECISION& nY, const PRECISION& nZ)
{
...
template<class DATA, class PRECISION, class SOURCEMASK, class SOURCECOMBINE>
inline void Set(SourceMaskCombineOf<DATA, 3, PRECISION, SOURCEMASK,
    SOURCECOMBINE>& src, const PRECISION& nX, const PRECISION& nY, const PRECISION& nZ, const DATA& data)
{
