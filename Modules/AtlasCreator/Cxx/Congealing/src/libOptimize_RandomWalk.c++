...
//============================================================================
bool Optimize_RandomWalk(int cDimensions, double* vrParamMin, double* vrInitialScale, double fxnEval(double*, const double& rErrMin, const void*), const void* pData, double rErrThreshold, int cIterations)
{
  ...
  for (int nDimension = 0; nDimension < cDimensions; nDimension++)
    {
    vrd[nDimension] = Random(-vrInitialScale[nDimension],
        vrInitialScale[nDimension]) / (20);
    }
  ...
  for (nIteration = 0; nIteration < cIterations; nIteration++)
    ...
    if (rErr < rErrMin)
      ...
      if (nStep == 0)
        ...
        vrd[nStepDimension] = Random(-vrInitialScale[nStepDimension],
            vrInitialScale[nStepDimension]) / (20);
        nStep = 0;
        }
      }
    }
