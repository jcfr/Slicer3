...
class PaddedImageBaseOf: public SourceMemoryOf<DATA, DIMENSIONALITY>
{
protected:
  int m_cPad;
...
public:
  ...
  {
    return (_LINE + "Image2D" + LINE_ + (_INDENT + "padding:" + m_cPad + LINE_)
        + this->DescribeCommon());
  }
  ...
  void Pad(const SourceOf<DATA_IN, DIMENSIONALITY, PRECISION> &srcIn, int cPad, PaddingType pad =
      Repeat, bool bPrepared = false)
  {
    m_cPad = cPad;

    POINT ptStorage = srcIn.Size() + (2 * m_cPad);

    D("allocating %d", ptStorage.CVolume());
    ptStorage.Describe();
...
template<class DATA, int DIMENSIONALITY>
class PaddedImageOf: public PaddedImageBaseOf<DATA, DIMENSIONALITY>
{
private:
  PaddedImageOf()
  {
  }
  ; // no creating one of these!
};

template<class DATA>
class PaddedImageOf<DATA, 2> : public PaddedImageBaseOf<DATA, 2>
{
...
template<class DATA>
class PaddedImageOf<DATA, 3> : public PaddedImageBaseOf<DATA, 3>
{
  typedef int PRECISION;
public:
SOURCE_ACTUALS_3D  ;
