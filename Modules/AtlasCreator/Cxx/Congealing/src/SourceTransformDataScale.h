...
class SourceTransformDataScaleBaseOf: public SourceAccessorOf<DATA,
...
public:
  ...
  SourceTransformDataScaleBaseOf() :
    SourceAccessorOf<DATA, DIMENSIONALITY, PRECISION, SOURCE> (), m_rSlope(1.),
        m_rIntercept(0.)
  {
  }
  ...
  {
    return (_LINE + "SourceTransformDataScaleOf(" + DIMENSIONALITY + "D)"
        + LINE_ + (_INDENT + "slope " + m_rSlope + " intercept " + m_rIntercept
  ...
  void SetSlopeIntercept(const Real& rSlope, const Real& rIntercept)
  {
    m_rSlope = rSlope;
...
template<class DATA, int DIMENSIONALITY, class PRECISION, class SOURCE>
class SourceTransformDataScaleOf: public SourceTransformDataScaleBaseOf<DATA,
    DIMENSIONALITY, PRECISION, SOURCE>
{
};

template<class DATA, class PRECISION, class SOURCE>
class SourceTransformDataScaleOf<DATA, 2, PRECISION, SOURCE> : public SourceTransformDataScaleBaseOf<
...
template<class DATA, class PRECISION, class SOURCE>
class SourceTransformDataScaleOf<DATA, 3, PRECISION, SOURCE> : public SourceTransformDataScaleBaseOf<
