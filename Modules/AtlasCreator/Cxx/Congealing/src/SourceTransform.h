...
class SourceTransformOf: public SourceOf<DATA, DIMENSIONALITY, PRECISION>
{
protected:
  SOURCE* m_psource;
...
protected:
  ...
  SourceTransformOf(const SourceTransformOf<DATA, DIMENSIONALITY, PRECISION,
      SOURCE>&src) :
...
public:
  ...
  {
    return (_LINE + "SourceTransformOf(" + DIMENSIONALITY + "D)" + LINE_
        + this->DescribeCommon());
  }
...
inline void Get(const SourceTransformOf<DATA, 3, PRECISION, SUBSOURCE>& src, DATA& dataOut, const PRECISION& nX, const PRECISION& nY)
{
  src.VirtualGet(dataOut, nX, nY);
}

template<class DATA, class PRECISION, class SUBSOURCE>
inline void Set(SourceTransformOf<DATA, 3, PRECISION, SUBSOURCE>& src, const PRECISION& nX, const PRECISION& nY, const DATA& data)
{
  src.VirtualSet(nX, nY, data);
}

template<class DATA, class PRECISION, class SUBSOURCE>
inline void Get(const SourceTransformOf<DATA, 3, PRECISION, SUBSOURCE>& src, DATA& dataOut, const PRECISION& nX, const PRECISION& nY, const PRECISION& nZ)
{
  src.VirtualGet(dataOut, nX, nY, nZ);
}

template<class DATA, class PRECISION, class SUBSOURCE>
inline void Set(SourceTransformOf<DATA, 3, PRECISION, SUBSOURCE>& src, const PRECISION& nX, const PRECISION& nY, const PRECISION& nZ, const DATA& data)
{
