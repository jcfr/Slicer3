...
class SourceAccessorEqualizeBaseOf: public SourceAccessorOf<DATA,
...
public:
  ...
  {
    return (_LINE + "SourceAccessorEqualizeOf(" + DIMENSIONALITY + ")" + LINE_
        + this->DescribeCommon());
  }
  ...
  void PrepareForAccessAction() const
  {
    ...
    for (int n = 0; n < c; n++)
      {
      m_vdataRemap[n] = (vcdata[n] + (n > 0 ? vcdata[n - 1] : 0)) / 2
          / cdataPerBin;
      }
  }
};
...
template<class DATA, int DIMENSIONALITY, class PRECISION, class SOURCE>
class SourceAccessorEqualizeOf: public SourceAccessorEqualizeBaseOf<DATA,
    DIMENSIONALITY, PRECISION, SOURCE>
{
};

template<class DATA, class PRECISION, class SOURCE>
class SourceAccessorEqualizeOf<DATA, 2, PRECISION, SOURCE> : public SourceAccessorEqualizeBaseOf<
...
template<class DATA, class PRECISION, class SOURCE>
class SourceAccessorEqualizeOf<DATA, 3, PRECISION, SOURCE> : public SourceAccessorEqualizeBaseOf<
