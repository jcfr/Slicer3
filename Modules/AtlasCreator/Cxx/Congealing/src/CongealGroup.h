...
class CongealGroupOf
{
  ...
  static const int DIMENSIONALITY = TypeOfDimensionality(RECIPIE);typedef TypeOfPrecision(RECIPIE) PRECISION;
...
public:
  ...
  {
    return ((CongealGroupOf<RECIPIE>*) pgroup)->StatisticalAlignmentError(
        vparam, rErrMin);
  }
  ...
  int cSamples, Real rThreshold = .001, int cIterations = 1000)
  {
    m_regParams.Clear();
    m_regSteps.Clear();
    ...
    ASSERTf(m_regParams.C() == m_regSteps.C(),
    ...
    UD("WITH %d MEASUREMENTS (%2.4g%%)", m_cStatisticalMeasurements, Real(100
        * m_cStatisticalMeasurements) / CSize());

    // NelderMeadOptimize
    // OptimizeGreedySwarm
    Optimize_RandomWalk(cParams, vparam, m_regSteps.V(),
        StatisticalAlignmentErrorOfGroup, this, rThreshold, cIterations);
  ...
  int cSamples, Real rThreshold = .001, int cIterations = 1000)
  {
    ...
    // OptimizeGreedySwarm
    //Optimize_RandomWalk
    (cParams, vparam, m_regSteps.V(), StatisticalAlignmentErrorOfGroup, this,
