...
class SourceAccessorOf: public SourceTransformOf<DATA, DIMENSIONALITY,
    PRECISION, SOURCE>
{
public:
  SourceAccessorOf() :
  ...
  {
    return (_LINE + "SourceAccessorOf" + LINE_ + this->DescribeCommon());
  }
...
inline void Get(const SourceAccessorOf<DATA, 2, PRECISION, SUBSOURCE>& src, DATA& dataOut, const PRECISION& nX, const PRECISION& nY)
{
  src.VirtualGet(dataOut, nX, nY);
}

template<class DATA, class PRECISION, class SUBSOURCE>
inline void Set(SourceAccessorOf<DATA, 2, PRECISION, SUBSOURCE>& src, const PRECISION& nX, const PRECISION& nY, const DATA& data)
{
  src.VirtualSet(nX, nY, data);
}

template<class DATA, class PRECISION, class SUBSOURCE>
inline void Get(const SourceAccessorOf<DATA, 3, PRECISION, SUBSOURCE>& src, DATA& dataOut, const PRECISION& nX, const PRECISION& nY, const PRECISION& nZ)
{
  src.VirtualGet(dataOut, nX, nY, nZ);
}

template<class DATA, class PRECISION, class SUBSOURCE>
inline void Set(SourceAccessorOf<DATA, 3, PRECISION, SUBSOURCE>& src, const PRECISION& nX, const PRECISION& nY, const PRECISION& nZ, const DATA& data)
{
