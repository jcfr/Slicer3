...
class SourceCombineOf: public SourceOf<DATA, DIMENSIONALITY, PRECISION>
{
...
protected:
  SourceCombineOf() :
    SourceOf<DATA, DIMENSIONALITY, PRECISION> (), m_vecpsrc()
  {
  }
...
public:
  ...
  {
    return (_LINE + "SourceCombineOf(" + DIMENSIONALITY + "D)" + LINE_
        + this->DescribeCommon());
  }
  ...
  virtual String DescribeCommon() const
  {
    ...
    for (int n = 0; n < m_vecpsrc.C(); n++)
      {
      s = s + (_INDENT + (_LINE + "source: " + n + LINE_ + (_INDENT
          + this->m_vecpsrc[n]->Describe() + LINE_)));
      }

    return _INDENT + (_LINE + "size: " + this->Size().Describe() + LINE_ + s);
  }
...
inline void Get(const SourceCombineOf<DATA, 3, PRECISION, SUBSOURCE>& src, DATA& dataOut, const PRECISION& nX, const PRECISION& nY)
{
  src.VirtualGet(dataOut, nX, nY);
}

template<class DATA, class PRECISION, class SUBSOURCE>
inline void Set(SourceCombineOf<DATA, 3, PRECISION, SUBSOURCE>& src, const PRECISION& nX, const PRECISION& nY, const DATA& data)
{
  src.VirtualSet(nX, nY, data);
}

template<class DATA, class PRECISION, class SUBSOURCE>
inline void Get(const SourceCombineOf<DATA, 3, PRECISION, SUBSOURCE>& src, DATA& dataOut, const PRECISION& nX, const PRECISION& nY, const PRECISION& nZ)
{
  src.VirtualGet(dataOut, nX, nY, nZ);
}

template<class DATA, class PRECISION, class SUBSOURCE>
inline void Set(SourceCombineOf<DATA, 3, PRECISION, SUBSOURCE>& src, const PRECISION& nX, const PRECISION& nY, const PRECISION& nZ, const DATA& data)
{
