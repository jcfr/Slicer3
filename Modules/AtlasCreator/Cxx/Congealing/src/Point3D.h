...
public:
  ...
  PointOf<3, PRECISION>& AdvanceLoop(const PointOf<3, PRECISION>& ptBy = 1)
  {
    m_xX += ptBy.m_xX;
    return *this;
  }

  bool StillLooping(const PointOf<3, PRECISION>& ptTo, const PointOf<3,
      PRECISION>& ptFrom = 0, const PointOf<3, PRECISION>& ptBy = 1)
  {
    if (m_xX < ptTo.X())
      {
      return true;
  ...
  PointOf<3, PRECISION> Bound(const PointOf<3, PRECISION> &ptMin, const PointOf<
      3, PRECISION> &ptMax) const
  {
    return PointOf<3, PRECISION> (::Bound(X(), ptMin.X(), ptMax.X()), ::Bound(
        Y(), ptMin.Y(), ptMax.Y()), ::Bound(Z(), ptMin.Z(), ptMax.Z()));
  }
...
//============================================================================
template<class PRECISION>
inline PointOf<3, PRECISION> NewRandomPoint(const PointOf<3, PRECISION> &pt)
{
  return PointOf<3, PRECISION> (Random(PRECISION(0), pt.X()), Random(PRECISION(
      0), pt.Y()), Random(PRECISION(0), pt.Z()));
}
