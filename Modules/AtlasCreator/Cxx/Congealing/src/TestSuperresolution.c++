...
void SuperRes()
{
  ...
  GreyImage *pi = ClaimPointer(ReadPGM(ConfigValue("inputfile",
      "../data/incrop0.pgm")));
  ...
  const InterpolationType it = InterpolationType(
      ConfigValue("interpolation", 0));
  const int cBlocksize = ConfigValue("blocksize", 8);

  D(
      "exploding, blurring, maxing and converting back into a maximum a posteriori image");
  ...
  //--------------------------------------------------------
  D("writing output");
  const char *sflOut = ConfigValue("outputfile",
      "../output/superres/superres.pgm");
