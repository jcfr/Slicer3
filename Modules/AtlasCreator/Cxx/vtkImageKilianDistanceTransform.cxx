...
void vtkImageKilianDistanceTransform::ExecuteInformation(vtkImageData *vtkNotUsed(input), vtkImageData *output)
{
...
// is always floats.
template<class TT>
void vtkImageKilianDistanceTransformCopyData(vtkImageKilianDistanceTransform *self, vtkImageData *inData, TT *inPtr, vtkImageData *outData, int outExt[6], float *outPtr)
{
  ...
  // Reorder axes
  self->PermuteExtent(outExt, outMin0, outMax0, outMin1, outMax1, outMin2,
      outMax2);
...
int IsInsideBoundary(int idx0, int outMin0, int outMax0, int inInc0, T* inPtr0, int idx1, int outMin1, int outMax1, int inInc1, int idx2, int outMin2, int outMax2, int inInc2, int objectLabel)
{
  ...
  if (*inPtr0 != T(objectLabel))
    return 0;
  // X Axis
  if ((idx0 > outMin0) && (*(inPtr0 - inInc0) != T(objectLabel)))
    return 1;
  if ((idx0 < outMax0) && (*(inPtr0 + inInc0) != T(objectLabel)))
    return 1;

  // Y Axis
  if ((idx1 > outMin1) && (*(inPtr0 - inInc1) != T(objectLabel)))
    return 1;
  if ((idx1 < outMax1) && (*(inPtr0 + inInc1) != T(objectLabel)))
    return 1;

  // Z Axis  
  if ((idx2 > outMin2) && (*(inPtr0 - inInc2) != T(objectLabel)))
    return 1;
  if ((idx2 < outMax2) && (*(inPtr0 + inInc2) != T(objectLabel)))
    return 1;
...
// is always floats.
template<class T>
void vtkImageKilianDistanceTransformInitialize(vtkImageKilianDistanceTransform *self, vtkImageData *inData, T *inPtr, vtkImageData *outData, int outExt[6], float *outPtr)
{
  //int inInc0, inInc1, inInc2;
  vtkIdType inInc0, inInc1, inInc2;
            ...
            if (signedDistanceMap)
              ...
              if (insideBoundaryZero)
                *outPtr0 = maxDist;
                ...
                if (IsOutsideBoundary(idx0, outMin0, outMax0, inInc0, inPtr0,
                    idx1, outMin1, outMax1, inInc1, idx2, outMin2, outMax2,
                    inInc2, objectValue))
                  *outPtr0 = 0;
                else
                  *outPtr0 = maxDist;
              ...
              *outPtr0 = 0; // it is not a signed distance map -> everything outside is set to zero
            }
          else
            {
            if (insideBoundaryZero && IsInsideBoundary(idx0, outMin0, outMax0,
                inInc0, inPtr0, idx1, outMin1, outMax1, inInc1, idx2, outMin2,
                outMax2, inInc2, objectValue))
              *outPtr0 = 0;
            else
              *outPtr0 = maxDist;
            }
  ...
  // No initialization required. We just copy inData to outData.
    {
    vtkImageKilianDistanceTransformCopyData(self, inData, (T *) (inPtr),
...
//
void vtkImageKilianDistanceTransformExecuteSaito(vtkImageKilianDistanceTransform *self, vtkImageData *outData, int outExt[6], float *outPtr)
{
...
//
void vtkImageKilianDistanceTransformExecuteSaitoCached(vtkImageKilianDistanceTransform *self, vtkImageData *outData, int outExt[6], float *outPtr)
{
...
template<class T>
void vtkImageKilianDistanceTransformDefineSignedDistanceMap(vtkImageKilianDistanceTransform *self, vtkImageData *inData, T *inPtr, vtkImageData *outData, int outExt[6], float *outPtr)
{
  int inInc0, inInc1, inInc2;
  T *inPtr0, *inPtr1, *inPtr2;
  int outMin0, outMax0, outMin1, outMax1, outMin2, outMax2;
  ...
  outMin2 = outExt[4];
  outMax2 = outExt[5];
  outInc0 = outData->GetIncrements()[0];
  outInc1 = outData->GetIncrements()[1];
  outInc2 = outData->GetIncrements()[2];
  inInc0 = inData->GetIncrements()[0];
  inInc1 = inData->GetIncrements()[1];
  inInc2 = inData->GetIncrements()[2];
  ...
  for (int idx2 = outMin2; idx2 <= outMax2; ++idx2)
    ...
    for (int idx1 = outMin1; idx1 <= outMax1; ++idx1)
      ...
      for (int idx0 = outMin0; idx0 <= outMax0; ++idx0)
        {
        if ((*inPtr0 != T(objectValue)) && (*outPtr0 > 0.0))
          *outPtr0 = -*outPtr0;
        inPtr0 += inInc0;
        outPtr0 += outInc0;
        }
...
void vtkImageKilianDistanceTransform::IterativeExecuteData(vtkImageData *inData, vtkImageData *outData)
{
  ...
  if (this->GetIteration() == 0)
    ...
    switch (inData->GetScalarType())
      {
      vtkTemplateMacro6(vtkImageKilianDistanceTransformInitialize, this,
  ...
  if (inData != outData)
    switch (inData->GetScalarType())
      {
      vtkTemplateMacro6(vtkImageKilianDistanceTransformCopyData, this, inData,
          (VTK_TT *) (inPtr), outData, outExt, (float *) (outPtr));
    }
}
...
case VTK_EMKILIAN_EDT_SAITO:
  vtkImageKilianDistanceTransformExecuteSaito(this, outData, outExt,
      (float *) (outPtr));
  break;
case VTK_EMKILIAN_EDT_SAITO_CACHED:
  vtkImageKilianDistanceTransformExecuteSaitoCached(this, outData, outExt,
      (float *) (outPtr));
  break;
default:
...
int vtkImageKilianDistanceTransform::SplitExtent(int splitExt[6], int startExt[6], int num, int total)
{
  int splitAxis;
  int min, max;

  vtkDebugMacro("SplitExtent: ( " << startExt[0] << ", " << startExt[1] << ", "
      << startExt[2] << ", " << startExt[3] << ", " << startExt[4] << ", "
      << startExt[5] << "), " << num << " of " << total);
  ...
  if (num >= total)
    {
    vtkDebugMacro("  SplitRequest (" << num << ") larger than total: " << total);
    return total;
    }
  ...
  if (num == total - 1)
    ...
    {
    splitExt[splitAxis * 2 + 1] = (min - 1) + (max - min + 1) * (num + 1)
        / total;
    }

  vtkDebugMacro("  Split Piece: ( " << splitExt[0] << ", " << splitExt[1]
      << ", " << splitExt[2] << ", " << splitExt[3] << ", " << splitExt[4]
      << ", " << splitExt[5] << ")");
  fflush( stderr);
...
void vtkImageKilianDistanceTransform::PrintSelf(ostream& os, vtkIndent indent)
{
  ...
  os << indent << "Initialize: " << (this->Initialize ? "On\n" : "Off\n");
  ...
  os << indent << "Zero Boundary Value:"
      << (this->InsideBoundaryZero ? "Inside\n" : "Outside\n");
  os << indent << "DistanceFormat:" << (this->DistanceFormat
      == VTK_EMKILIAN_EDT_SQUARE_ROOT ? "Square Root\n" : "Euclidean\n");
