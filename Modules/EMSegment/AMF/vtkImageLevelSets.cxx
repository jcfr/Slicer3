/*=auto=========================================================================
  ...
  Program:   3D Slicer
  Module:    $RCSfile: vtkImageLevelSets.cxx,v $
  Date:      $Date: 2007/04/04 19:43:10 $
...
//
//------------------------------------------------------------------------------
vtkImageLevelSets* vtkImageLevelSets::New()
{
  // First try to create the object from the vtkObjectFactory
  vtkObject* ret = vtkObjectFactory::CreateInstance("vtkImageLevelSets");
  if(ret)
    {
    return (vtkImageLevelSets*)ret;
    }
  // If the factory was unable to create the object, then create it here.
  return new vtkImageLevelSets;
}


//----------------------------------------------------------------------------
// Constructor sets default values
vtkImageLevelSets::vtkImageLevelSets()
{
  verbose = 0;
  ...
  */
  ...
  balloon_coeff        = 0;
  // Same as ballon_image but not spatially changed 
  balloon_value        = 0.0;
  balloon_image        = NULL;
  NumGaussians         = 0;
  Gaussians            = NULL;
  ...
  this->logCondIntensityImage = NULL;
  this->logCondIntensityCoefficient = 0.0;
  ...
  // when probCondWeightMin is set to 1 then the term is ignored ! 
  this->probCondWeightMin = 1;
...
//---------------------------------------------------------------------------
void vtkImageLevelSets::SetNumInitPoints( int n)
//
{
  int i;
...
//---------------------------------------------------------------------------
void vtkImageLevelSets::SetInitPoint( int num, int x, int y, int z, int radius)
//
{
...
//---------------------------------------------------------------------------
void vtkImageLevelSets::SetNumGaussians( int n)
//
{
  int i;
...
//---------------------------------------------------------------------------
void vtkImageLevelSets::SetGaussian( int num, float mean, float sd)
//
{
...
//---------------------------------------------------------------------------
void vtkImageLevelSets::DistanceMap()
//                   -----------
{
  // Save the input
  if (savedistmap)
...
//   
//
void vtkImageLevelSets::DistanceMapCurves()
//                   ------------
{
...
//   
//
void vtkImageLevelSets::DistanceMapFMOld()
//                   ------------
{
   float*        U    = u[this->current];
   float*        newU = u[1-this->current];
...
//   
//
void vtkImageLevelSets::DistanceMapFM()
//                   ------------
{
   float*        U    = u[this->current];
   float*        newU = u[1-this->current];
...
//
//
void vtkImageLevelSets::DistanceMapChamfer()
//                   ------------
{
...
//
//
void vtkImageLevelSets::DistanceMapShape()
//                 ----------------
{
   float*        U    = u[this->current];
   float*        newU = u[1-this->current];
...
// void vtkImageLevelSets::SetDMmethod_LogOddsMap(vtkImageData* LogOddsInput) {
//   cout << "vtkImageLevelSets::SetDMmethod_LogOddsMap: Set DMmethod to LogOddsMap - make sure  to put this method at the end of defining your level set parameters - otherwise you overwrite log odds defaults !" << endl;
//   // Set variables 
//   this->DMmethod = DISTMAP_LOGODDS ;
// 
//   // Copy over log odds map through defining initImage 
//   // Define zero level set  
//   this->InitThreshold = 0; 
//   // Positive values are the inside of the log odds map 
//   // => map will be negated in InitEvolution() to follow level set notation
//   this->InitIntensity = Bright;
//   // Set the Initial Image with the LogOdds map 
//   this->SetinitImage(LogOddsInput);
// 
//   // Band Parameter does not work bc we cannot recompute logOdds ! 
//   // this->Band = 100;
//   // this->Tube = 95; 
//         
//   // Currently want to experiment just what happens when running level set on log odds 
//   // => Set all other coefficients to zero  
//   this->coeff_curvature = 1.0;
//   this->AdvectionCoeff = 0.0; 
//   this->balloon_coeff = 0.0;
//   if (this->velocity != NULL) {
//     cout << "Error:vtkImageLevelSets::SetDMmethod_LogOddsMap: velocity has to be zero ! " <<  endl;
//     exit(1);
//   }
// 
...
//   // this->verbose = 1;
// 
// } // LogOddsMap(vtkImageData* LogOddsInput)
...
unsigned char vtkImageLevelSets::CheckConvergence( )
{
   ...
   if (GB_debug) fprintf(stderr, "vtkImageLevelSets::CheckConvergence( ) end \n");
...
//--------------------------------------------------------------------------
// Check the convergence.
void vtkImageLevelSets::CheckConvergenceNew( )
{
  ...
  if (!bnd_allocated)
    return;
  if (GB_debug) fprintf(stderr, "checked vtkImageLevelSets::CheckConvergenceNew( ) begin \n");
  ...
  if (GB_debug) fprintf(stderr, "vtkImageLevelSets::CheckConvergenceNew( ) end \n");
...
//---------------------------------------------------------------------------
//
inline void vtkImageLevelSets::ExtractCoords(int p, short& x, short& y, short& z)
{
  x = p%tx;
  p = (p-x)/tx;
  y = p%ty;
...
//---------------------------------------------------------------------------
//
void vtkImageLevelSets::MakeBand( )
{
...
//
//
void vtkImageLevelSets::MakeBand0( )
{
    if (verbose) cout << "Checked -chemper - does not change value in band  vtkImageLevelSets::MakeBand0 " << endl;
    register int   p;
    register int   x,y,z;
    register float *U =this->u[  this->current];
  ...
  if (!bnd_allocated)     {
    ...
    bnd_allocated           = 1;
    ADDMEMORY("vtkImageLevelSets::MakeBand0() band size bnd + flag ", bnd_maxsize*sizeof(int)+imsize)
    //    flag_allocated = 1;
  }
  ...
  case DISTMAP_LOGODDS:
    ...
    // Kilian : Changed this from original 
    xmin = 1;
    xmax = tx-2; 
    ymin = 1;   
    ymax = ty-2;
...
//                   ---------
{
  ...
  if (verbose) {
    fprintf(stderr,"check vtkImageLevelSets::InitParam() \n");
  ...
  if (inputImage == NULL) {
    ...
    ADDMEMORY("vtkImageLevelSets::InitParam() Input & Output Images",(2*sizeof(float)*imsize));
  ...
  ADDMEMORY("vtkImageLevelSets::InitParam() stored_seg (UNSIGNED CHAR)",imsize);
...
//---------------------------------------------------------------------------
void vtkImageLevelSets::ResizeBand()
//                   ----------
{
  int*           new_band;
  float*         new_bandval;
  ...
  */
  ...
  ADDMEMORY("vtkImageLevelSets::ResizeBand() size increase (INT)",(new_maxsize-bnd_maxsize)*sizeof(int));
...
//
//
void vtkImageLevelSets::Evolve2D()
//                   --------
{
    if (GB_debug) cout << "vtkImageLevelSets::Evolve2D Start" << endl;
    short i,j,k;
    int   b;
    int   p; 
  ...
  if (GB_debug) {
    fprintf(stderr, "vtkImageLevelSets::Evolve2D( ) .");
    fflush(stderr);
  }
  ...
  for (b=0;b<this->bnd_pc;b++){
    ...
    //--------------------------------------------------
    // LogConditional Intensity Term from IPMI07
    //--------------------------------------------------
    float logConditionalIntensityTerm =0.0;

    // Binomial sig function -> Turn log odds into probabilities - remember that u0 has opposite sign of how we define it in the paper
    float q = 1.0/(1.0 + exp(u0));
    float probCondWeight = q * (1.0 - q);
    // By default this->probCondWeightMin = 1 => it is not considered 
    if (probCondWeight < this->probCondWeightMin )  probCondWeight = this->probCondWeightMin;
 
    if ((this->logCondIntensityCoefficient != 0.0) && this->logCondIntensityImage) {
      // Have to multiply with logCondIntensityInside to make sure that the inside is negative 
      logConditionalIntensityTerm = this->logCondIntensityCoefficient *probCondWeight*( u0 - logCondIntensityInside * logIntensityImagePtr[p]); 
    } 
    ...
    //--------------------------------------------------
      ...
      else  
      {
        ...
        } // end if (fabs(AdvectionCoeff)>1E-10) 
      } // end if (normcompsq==0) ... else ...
    ...
    if (fabs(balloon_coeff)>1E-10) {
      balloonterm = this->balloon_value;
      if (balloonterm == 0.0) {
    ...
    //--------------------------------------------------
      ...
      // Change from berfore
      // Kilian
      if (curvature_data!=NULL) curvature_data[p] = probCondWeight*curvterm;
      if (advection_data!=NULL) advection_data[p] = -probCondWeight*imcomp;
      if (velocity_data!=NULL)  velocity_data [p] = -probCondWeight*vel;
      if (balloon_data !=NULL)  balloon_data  [p] = -probCondWeight*balloonterm;
      // Kilian : Here is the change 
      ut = probCondWeight*(curvterm -imcomp - balloonterm - vel);
    ...
    if (this->logCond_data) this->logCond_data[p] = -logConditionalIntensityTerm;
    this->mean_logCond   += logConditionalIntensityTerm;
  ...
  // Kilian Debug - so it works with multi label case 
  // this->current=1-this->current;
  memcpy(u[this->current],u[1 - this->current],imsize*sizeof(float));

  if (this->touched) {
    fprintf(stderr,"vtkImageLevelSets::Evolve2D: Curve touched Band \n");
    if (this->DMmethod == DISTMAP_LOGODDS){
      fprintf(stderr,"ERROR:vtkImageLevelSets::Evolve2D: Cannot reintialize Curve in LogOdds Mode! Expand tube (and band) ! \n");
      exit(1);
    }
    //    fflush(stderr);
    this->DistanceMap();
  ...
  this->mean_logCond   /= bnd_pc;

  if (verbose) {
    fprintf(stderr,"Evolve2D()\n");
    fprintf(stderr," MEAN curv      = %2.5f\n",this->mean_curv);
    fprintf(stderr," MEAN Log Cond  = %2.5f\n",this->mean_logCond);
    fprintf(stderr," MEAN advection = %2.5f\n",this->mean_advection);
    fprintf(stderr," MEAN balloon   = %2.5f\n",this->mean_balloon);
    fprintf(stderr," MEAN velocity  = %2.5f\n",this->mean_vel);
...
//---------------------------------------------------------------------------
VTK_THREAD_RETURN_TYPE vtkImageLevelSetsThreadedEvolve3D( void *arg )
{
    vtkImageLevelSets* This;
  ...
  This = (vtkImageLevelSets *)(((ThreadInfoStruct *)(arg))->UserData);

  total = This->SplitBand(first, last, threadId, threadCount);
  // cout << "vtkImageLevelSetsThreadedEvolve3D " << threadId << " " << total  << " " << first << " " << last << " " << threadId << " " << threadCount << "==== " << endl;
  if (threadId < total) This->Evolve3D(first,last);
  
  return VTK_THREAD_RETURN_VALUE;

} // vtkImageLevelSetsThreadedEvolve3D()


//---------------------------------------------------------------------------
void vtkImageLevelSets::Evolve3D( )
//
{
  this->touched=0;
  ...
  if (this->EvolveThreads<=0)
    ...
    threader->SetSingleMethod(vtkImageLevelSetsThreadedEvolve3D, this);
...
//
int vtkImageLevelSets::SplitBand(int& first, int& last, int num, int total)
{
  ...
  // cout << this->bnd_pc << " --- " << maxThreadIdUsed << endl;
...
//
//
void vtkImageLevelSets::Evolve3D( int first_band, int last_band)
//                   --------
{
    int   b;
    int   p; 
    // cout << "Evolve3D " << first_band << " " << last_band << endl;
  ...
  for (b=first_band;b<=last_band;b++){
   ...
   //--------------------------------------------------
    ...
    // Binomial sig function -> Turn log odds into probabilities
    float q = 1.0/(1.0 + exp(u0));
    float probCondWeight = q * (1.0 - q);
    if (probCondWeight < this->probCondWeightMin )  probCondWeight = this->probCondWeightMin;

    if ((this->logCondIntensityCoefficient != 0.0) && this->logCondIntensityImage) {
      // Binomial sig function
      // float q = 1.0/(1.0 + exp(u0));
      // Have to multiply with logCondIntensityInside to make sure that the inside is negative 
      logConditionalIntensityTerm = this->logCondIntensityCoefficient * ( u0 - logCondIntensityInside * logIntensityImagePtr[p]); 
    } 
    ...
    if (fabs(balloon_coeff)>1E-10) {
      // Kilian  : changed default a little bit
      balloonterm = this->balloon_value;
      if (balloonterm == 0.0) {
        cout << " HEEEEEEEEEEEEEEE: Carefull - in log odds no image information!!!!" << endl;
        if (balloon_image != NULL) 
          balloonterm = ((float*)balloon_image->GetScalarPointer())[p];
        else 
          balloonterm = this->ExpansionMap(im[p]);
...
// try another scheme ...
    ...
    //--------------------------------------------------
      ...
      if (curvature_data!=NULL) curvature_data[p] = probCondWeight*curvterm*StepDt;
      if (advection_data!=NULL) advection_data[p] = -probCondWeight*imcomp*StepDt;
      if (velocity_data !=NULL) velocity_data [p] = - probCondWeight*vel*StepDt;
      if (balloon_data  !=NULL) balloon_data  [p] = - probCondWeight*balloonterm*StepDt;


      //ut = min(max(StepDt*ut,-Band),Band);
      // cout << ut << endl;
      newU[p]=u0+ probCondWeight*StepDt*ut;
...
//----------------------------------------------------------------------------
void vtkImageLevelSets::InitEvolution()
{
  ...
  if (verbose) {
    fprintf(stderr,"checked vtkImageLevelSets::InitEvolution() \n");
    this->PrintParameters();
  }
  ...
  if ((isotropic_voxels)&&(verbose))
   ...
   if (initImage == NULL) {
     ...
     if (this->DMmethod == DISTMAP_LOGODDS) {
       // LogOdds has to be initialized 
       cout << "ERROR:vtkImageLevelSets::InitEvolution: InitImage has to be set if DistanceMap Type is LogOdds !" << endl; 
       exit(1);
     }
     if (NumInitPoints>0) {
  ...
  } // end  if (initImage == NULL) 


  this->current = 0;
  // u[0] Initial level set 
  this->u[0]    = (float*) outputImage->GetScalarPointer();
  this->u[1]    = new float [imsize];

  ADDMEMORY("vtkImageLevelSets::InitEvolution() u[1]",imsize*sizeof(float));
  ...
  if (fabs(AdvectionCoeff)>1E-10) { 
    ...
    case ADVECTION_CENTRAL_VECTORS:
      ...
      ADDMEMORY("vtkImageLevelSets::InitEvolution() data_attach_{x,y}",2*sizeof(float)*imsize);
      if (Dimension==3) {
        this->data_attach_z = new float[imsize];
        ADDMEMORY("vtkImageLevelSets::InitEvolution() data_attach_z",sizeof(float)*imsize);
      }
      break;
    case ADVECTION_MORPHO:
      this->secdergrad    = new float[imsize];
      this->normgrad      = new float[imsize];
      ADDMEMORY("vtkImageLevelSets::InitEvolution() ADVECTION_MORPHO secdergrad",2*sizeof(float)*imsize);
      break;
    }
  }
...
int vtkImageLevelSets::Iterate()
{
  ...
  if ((step >0) &&(step % CheckFreq == 0)) {
    ...
    if (reinitcntr > 0) {
      this->DistanceMap();
      if (this->DMmethod != DISTMAP_LOGODDS ) {this->MakeBand();}
      reinitcntr=0;
    }
...
//----------------------------------------------------------------------------
void vtkImageLevelSets::PreComputeDataAttachment()
{
  ...
  if (advection_scheme == ADVECTION_MORPHO) {
    ...
    delete [] this->normgrad;
    this->normgrad=NULL;
    ADDMEMORY("vtkImageLevelSets::PreComputeDataAttachment()  delete normgrad",-1.0*sizeof(float)*this->imsize);
  }
...
//----------------------------------------------------------------------------
void vtkImageLevelSets::NormalizeSecDerGrad()
{
  float*         tmp;
  float*         tmp_buf;
  float*         sdg_buf;
  ...
  ADDMEMORY("vtkImageLevelSets::NormalizeSecDerGrad() tmp",(1.0*sizeof(float)*this->imsize));
...
// the data types.
void vtkImageLevelSets::ExecuteData( vtkDataObject *outData)
//                   -------
{


  if (GB_debug) fprintf(stderr,"check Execute begin \n");
...
//
int vtkImageLevelSets::UpdateResult(){
...
//
float *vtkImageLevelSets::GetInitPointsStatistics()
{
...
// within the initial spheres or disks
//
void vtkImageLevelSets::InitPointsStatistics( float stats[2])
{
...
void vtkImageLevelSets::ComputeProbabilityLUT()
{
  int i;
...
//
//
float vtkImageLevelSets::ExpansionMap( float I, unsigned char compute)
{
  int n;
  double tmp,res;
...
//----------------------------------------------------------------------
void vtkImageLevelSets::PrintParameters()
{
  int i;
...
//----------------------------------------------------------------------
void vtkImageLevelSets::PrintSelf(ostream& os, vtkIndent indent)
{
   ...
   os << indent << "coeff_velocity: "       << this->coeff_velocity   << "\n";
   os << indent << "balloon_coeff: "        << this->balloon_coeff     << "\n";
   os << indent << "balloon_value: "        << this->balloon_value     << "\n";
   os << indent << "coeff_curvature: "      << this->coeff_curvature         << "\n";
   os << indent << "logCondIntensityCoefficient: " << this->logCondIntensityCoefficient        << "\n";
   os << indent << "logCondIntensityInside: "      << this->logCondIntensityInside         << "\n";
   os << indent << "probCondWeightMin: "           << this->probCondWeightMin         << "\n";
   os << indent << "DMmethod: "             << this->DMmethod         << "\n";
