...
#define EXERCISE_BASIC_STORAGE_MRML_METHODS( className, node )   \
  {                                                 \
    ...
    TEST_SET_GET_STRING(node, TempFileName);        \
    ...
    const char *absPath = node->GetAbsoluteFilePath("tmp/file.txt");    \
    std::cout << "Get absolute path from tmp/file.txt: " << (absPath == NULL ? "NULL" : absPath) << std::endl; \
  }
